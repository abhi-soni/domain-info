    ...
    result === null
      ...
      : [
          ...
          // Checking if there are more than one values in result.status
          {
            label: "Domain Status",
            value:
              Array.isArray(result.status) && result.status.length > 1
                ? Object.entries(result.status).map((status) => {
          ...
          {
            label: "Whois Server",
            value: result.whoisserver || "Not Available",
          },
          {
            label: "Registry Expiry Date",
            value: result.expires || "Not Available",
          },
          { label: "Registered", value: result.registered ? "Yes" : "No" },
          {
            label: "DNSSEC",
            value:
              typeof result.dnssec === "boolean"
                ? result.dnssec
                  ? "Yes"
                  : "No"
                : result.dnssec,
          },
          {
            ...
            value:
              (result.registered && result.registrar.name) || "Not Available",
          },
          {
            label: "Registrar ID",
            value:
              (result.registered && result.registrar.id) || "Not Available",
          },
          {
            label: "Registrar Email",
            value:
              (result.registered && result.registrar.email) || "Not Available",
          },
          // { label: 'Registrar Email', value: result.registrar.email.length > 1 ? result.registrar.email.map((email, index) => { return <span key={index}>{email}<br /></span> }) || 'Not Available': result.registrar.email || 'Not Available' },
          {
            label: "Registrar URL",
            value:
              (result.registered && result.registrar.url) || "Not Available",
          },
          {
            label: "Registrar Phone",
            value:
              (result.registered && result.registrar.phone) || "Not Available",
          },
            ...
            >
              <Paper elevation={3}>
                ...
                >
                  ...
                  {Object.entries(result.contacts).length ===
                  0 /*check If there is no contact details*/ ? (
                    <Typography variant="subtitle1" gutterBottom>
                      Not Available
                    </Typography>
                  ) : (
                    /*If details are there, then showing them*/
                    Object.entries(result.contacts).map(
                        ...
                        >
                          ...
                          {contactTypeDetail.map((detail, index) => (
                            <AccordionDetails key={index}>
                              <Typography variant="subtitle1" gutterBottom>
                                Name: {detail.name || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Organization:{" "}
                                {detail.organization || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Email: {detail.email || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Phone: {detail.phone || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Type: {detail.type || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Address: {detail.address || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                City: {detail.city || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Zip Code: {detail.zipcode || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Country: {detail.country || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Fax: {detail.fax || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Creation Date:{" "}
                                {detail.created || "Not Available"}
                              </Typography>
                              <Typography variant="subtitle1" gutterBottom>
                                Updated Date:{" "}
                                {detail.changed || "Not Available"}
                              </Typography>
                            </AccordionDetails>
                          ))}
                        </Accordion>
                  ...
                  <pre style={{ whiteSpace: "break-spaces" }}>
                    {result.rawdata} <Copy value={result.rawdata} />
                  </pre>
                </Box>
              </Paper>
            </Grid>
